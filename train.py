...
def run_experiments_for_estimator(dataset_name, dataset_obj, estimator_name, estimator, model_f, seed, train_estimator_flag, save_dir, nuisance_stats_dir):

    print('Dataset: ', dataset_name)
    print('Seed: ', seed)
    print('ESTIMATOR Hyper param: ', model_f)
    print('Nuisance Model: ', nuisance_model)
    ...
    metrics = calculate_metrics(dataset_name, dataset_obj, estimator_name, estimator, nuisance_model_config, seed=seed, conf_ints=False, nuisance_stats_dir= nuisance_stats_dir)
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--N_SEEDS_CV', type=int, default=2,
    #                     help='Total seeds for cross validation')
    ...
    if args.automl_selection:
        ...
        if estimator_name in ['dml_learner', 'causal_forest_learner']:
            model_y = clone(pickle.load(open(nuisance_stats_dir + 'mu_r_score.p', "rb")))
        elif estimator_name in ['dr_learner', 'dr_learner_tune_0.1', 'dr_learner_tune_0.01', 's_learner',
                                's_learner_upd']:
            model_y = clone(pickle.load(open(nuisance_stats_dir + 'mu_s.p', "rb")))
    ...
    if os.path.isfile(save_location):
        ...
        # res_df.to_csv(save_location, float_format='%.2f', index=False)

        # old_arr= res_df['ite-estimates'][0]
        # temp= pickle.load(open(save_location, 'rb'))
        # new_arr= temp['ite-estimates'].to_numpy()[0]
        # print(np.sum(old_arr != new_arr))