...
def acic_dataset_details(root_dir, case='acic_2018'):
...
# res={'dataset':[], 'seed':[], 'train_size': [], 'eval_size': [], 'ite_mean':[] ,'ite_var': []}
# res={'dataset':[], 'seed':[], 'train_size': [], 'eval_size': [], 'data_dim': [], 'class_ratio_train':[], 'class_ratio_eval':[]}
res={'dataset':[], 'seed':[], 'train_size': [], 'eval_size': [], 'data_dim': []}
for seed in range(seed_range):
    for dataset_name in dataset_list:
        ...
        # res['class_ratio_train'].append( np.sum(train_t == 0) / np.sum(train_t ==1) )
        # res['class_ratio_eval'].append( np.sum(eval_t == 0) / np.sum(eval_t ==1) )
