...
def load_df(ensemble=False):
    ...
    if ensemble:
        df = pd.read_csv(os.getcwd() + '/results/' + '/logs-ensemble-old.csv')
    else:
        df = pd.read_csv(os.getcwd() + '/results/' + 'logs.csv')
...
def run_analysis(df,
                 ...
                 argmin_metrics=['pehe', 'rscore', 'value_score', 'value_dr_score',
                                 'value_dr_clip_prop_score', 'tau_t_score', 'tau_s_score', 'tau_match_score',
                                 'tau_iptw_score', 'tau_iptw_clip_prop_score', 'tau_dr_score', 'tau_dr_clip_prop_score',
                                 'influence_score',
    ...
    # normalized_agg_pehe_df.to_csv('pehe_pc_df.csv')
    # normalized_agg_pehe_df.transpose().to_csv('pehe_pc_df_trans.csv')
    ...
    if plot_chosen_ests:
        ...
        for argmin_metric, argminned_df in argminned_dfs.items():
            ...
            if argmin_metric not in ['tau_dr_score', 'rscore']:
                continue
            ...
            plt.tight_layout()
            plt.savefig('results/' + argmin_metric  + '.jpg')
